...
namespace AlgorithmCodeKata {
    ...
    // 상수는 여섯 번째 재료가 쌓였을 때, 세 번째 재료부터 여섯 번째 재료를 이용하여 햄버거를 포장하고, 아홉 번째 재료가 쌓였을 때, 두 번째 재료와 일곱 번째 재료부터 아홉 번째 재료를 이용하여 햄버거를 포장합니다.즉, 2개의 햄버거를 포장하게 됩니다.


    // 상수에게 전해지는 재료의 정보를 나타내는 정수 배열 ingredient가 주어졌을 때, 상수가 포장하는 햄버거의 개수를 return 하도록 solution 함수를 완성하시오.


    public class _068_Hamburger : KataBase {
        ...
        public int solution(int[] ingredient) {
            ...
            foreach (int i in ingredient) {
                list.Add(i);
                if (list.Count >= 4) {
                    if (list[^4] == 1 && list[^3] == 2 && list[^2] == 3 && list[^1] == 1) {
                        answer++;
        ...
        public override void Example() {
            ...
            Console.WriteLine($"결과: {solution(ingredient)}");
        }
