...
namespace AlgorithmCodeKata {
    ...
    public class _066_RoughKeyboard : KataBase {
        public int[] solution(string[] keymap, string[] targets) {
            ...
            for (int i = 0; i < targets.Length; i++) {
                ...
                for (int j = 0; j < targets[i].Length;j++) {
                    int _c = GetKeyCount(keymap, targets[i][j]);
                    if (_c == -1) {
                        count = _c;
                        break;
                    }
        ...
        public int GetKeyCount(string[] keymap, char c) {
            int index = 0;
            while (index < 100) {
                for (int i = 0; i < keymap.Length; i++) {
                    if (keymap[i].Length <= index) continue;
                    if (keymap[i][index] == c) return index + 1;
                }
                index++;
            }
            return -1;
        ...
        public override void Example() {
            string[] keymap = { "AA" };
            string[] targets = { "B", "A", "AB", "BA", "BB" };
            //string[] keymap = { "ABCD", "DCBA" };
            //string[] targets = { "A", "B", "C", "D" };
            var result = solution(keymap, targets);
