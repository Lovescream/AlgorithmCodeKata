...
namespace AlgorithmCodeKata {
    ...
    public class _072_RunRun : KataBase {
        public string[] solution(string[] players, string[] callings) {
            Dictionary<string, int> dictionary = players.Select((s, i) => new { name = s, index = i }).ToDictionary(d => d.name, d => d.index);

            for (int i = 0; i < callings.Length; i++) {
                string calling = callings[i];
                int rank = dictionary[calling];

                string temp = players[rank - 1];
                players[rank - 1] = calling;
                players[rank] = temp;

                dictionary[calling] = rank - 1;
        ...
        public override void Example() {
            ...
            string[] callings = new string[] { "b", "a", "b", "a" };
