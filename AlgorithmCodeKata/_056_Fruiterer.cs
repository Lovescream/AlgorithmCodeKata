...
namespace AlgorithmCodeKata {
    ...
    public class _056_Fruiterer {
        public int solution(int k, int m, int[] score) {
            //List<int> list = score.OrderByDescending(x => x).ToList();
            //int iik = 0;
            ...
            //        list.RemoveAt(0);
            //    }
            //    iik += box[box.Length - 1] * m;
            //}

            //return iik;
            int[] array = score.OrderByDescending(x => x).ToArray();
            List<int[]> boxes = new List<int[]>();
            int index = 0;
            while (index + m <= array.Length) {
                int[] box = new int[m];
                for (int i = 0; i < m; i++) {
                    box[i] = array[index++];
                }
